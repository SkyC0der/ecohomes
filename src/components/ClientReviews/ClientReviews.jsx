...
const ClientReviews = () => {
  ...
  // Autoplay: advance to next slide every 6s
  useEffect(() => {
    if (isAnimating) return;
    const timer = setTimeout(() => {
      const next = (activeClient + 1) % clientReviewsContent.length;
      handleClientClick(next);
    }, 6000);
    return () => clearTimeout(timer);
  }, [activeClient, isAnimating]); // eslint-disable-line react-hooks/exhaustive-deps
